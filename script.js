...
async function put(emailG, hashG) {
   const doc = {
      email: emailG,
      hash: hashG,
      data: {
   ...
   try {
      const response = await fetch('http://localhost:3000/putData', {
         method: 'POST',
         headers: {
      ...
      document.querySelector(".floating-prompt-container").style.visibility = "hidden";
   }
}
async function retrieve(emailG, hashG) {

   try {

      const response = await fetch('http://localhost:3000/retrieveData', {
         method: 'POST',
         headers: {
            'Content-Type': 'application/json'
         },
      ...
      if (response.ok) {
         ...
         atk1 = doc.attack_1
         atk2 = doc.attack_2
         wpn1.textContent = doc.weapon_1
         wpn2.textContent = doc.weapon_2
         hp.textContent = doc.hp_s
         pts.textContent = doc.pot_s
...
async function getfields() {
   ...
   try {
      hash = await generateHash(document.getElementById('password').value)
   } catch (error) {
...
document.addEventListener("DOMContentLoaded", function () {
   ...
   if (get) {
      get.addEventListener("click", function (event) {
         ...
         getfields();
         if (isLoad) {
            retrieve(email, hash).then().catch(error => { console.error('Error:', error); });
         } else {
            put(email, hash).then().catch(error => { console.error('Error:', error); });
         }
         hideFloatingPrompt()
      });
   }
...
but1.onclick = () => {
   nam = prompt('Enter character name\n(12 characters only)');
   plnam.textContent = nam.slice(0, 12);
   buttSave.disabled = false;
   Inn()
}
